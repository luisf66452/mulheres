...
2. Chama diretamente `adminClient.auth.admin.generateLink({ type: 'magiclink', email, options: { redirectTo: `${siteUrl}/auth/callback` } })` — **sem checar antes se a usuária já existe**: para `type: 'magiclink'`, a própria API do Supabase cria a usuária automaticamente se o e-mail ainda não tiver conta, e reaproveita a existente se já tiver (retornando `data.user` e o link em `data.properties.action_link` nos dois casos). Isso cobre tanto quem nunca teve conta quanto quem já tinha uma conta free e assinou direto pela landing, sem precisar de uma checagem de existência separada. O trigger `on_auth_user_created` já existente cria a linha em `perfis` automaticamente quando a usuária é nova (plano `free` por default) — não precisa inserir manualmente.
3. Com o `id` retornado em `data.user.id`, atualiza `perfis` com `stripe_customer_id`, `stripe_subscription_id` e `plano: 'premium'`/`assinatura_status: 'active'` — mesmos campos que o ramo autenticado já atualiza logo abaixo no mesmo handler.
4. Envia o `action_link` por e-mail via Resend, novo template `src/lib/email/assinatura.ts` (mesmo padrão de `email/ebook.ts`: função que retorna `boolean`, nunca lança — falha de e-mail não derruba o webhook). O e-mail explica que a assinatura foi ativada e traz o link de acesso, **e também menciona que dá pra entrar a qualquer momento em `/login` com o mesmo e-mail** — rede de segurança caso o link falhe.

Falha ao gerar o link (passo 2) é logada e interrompe só esse fluxo de e-mail — mas isso significa que `perfis` não seria atualizado nesse caso; por isso o passo 3 só roda se o passo 2 retornou um `data.user.id` válido. Falha ao enviar o e-mail (passo 4) é só logada, não interrompe o webhook — a promoção a premium (passo 3) já aconteceu, e o acesso via `/login` continua funcionando independentemente do e-mail.
